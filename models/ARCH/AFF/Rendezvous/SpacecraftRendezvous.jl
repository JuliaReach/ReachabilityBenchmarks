...
- `abort_time` -- (optional, default: `-1.`) time of abort; can be either a
                  ...
                  *no-abort* scenario
"""
function spacecraft(; abort_time::Union{Float64,Vector{Float64}}=-1.0)
    # variables
    ...
    invariant = HalfSpace(sparsevec([x], [1.0], n), -100.0)  # x <= -100
    if aborting
        invariant = HPolyhedron([invariant,
                                 HalfSpace(sparsevec([t], [1.0], n), t_abort_upper)])
    end
    ...
    invariant = HPolyhedron([HalfSpace(sparsevec([x], [-1.0], n), 100.0),           # x >= -100
                             ...
                             HalfSpace(sparsevec([x, y], [-1.0, 1.0], n), 141.1)])  # -x + y <= 141.1
    if aborting
        addconstraint!(invariant, HalfSpace(sparsevec([t], [1.0], n), t_abort_upper))  # t <= t_abort_upper
    end
    ...
    if aborting
        ...
        guard = HalfSpace(sparsevec([t], [-1.0], n), -t_abort_lower)  # t >= t_abort_lower
    ...
    X0 = Hyperrectangle([-900.0, -400.0, 0.0, 0.0, 0.0],
    ...
    octagon = [HalfSpace(sparsevec([vx], [-1.0], n), cx),                 # vx >= -cx
               HalfSpace(sparsevec([vx], [1.0], n), cx),                  # vx <= cx
               HalfSpace(sparsevec([vy], [-1.0], n), cx),                 # vy >= -cx
               HalfSpace(sparsevec([vy], [1.0], n), cx),                  # vy <= cx
               HalfSpace(sparsevec([vx, vy], [1.0, 1.0], n), cy + cx),    # vx + vy <= cy + cx
               HalfSpace(sparsevec([vx, vy], [1.0, -1.0], n), cy + cx),   # vx - vy <= cy + cx
               HalfSpace(sparsevec([vx, vy], [-1.0, 1.0], n), cy + cx),   # -vx + vy <= cy + cx
               HalfSpace(sparsevec([vx, vy], [-1.0, -1.0], n), cy + cx)]  # -vx - vy <= cy + cx
    tan30 = tan(π / 6)
    cone = [HalfSpace(sparsevec([x], [-1.0], n), 100.0),          # x >= -100
            HalfSpace(sparsevec([x, y], [tan30, -1.0], n), 0.0),  # -x tan(30°) + y >= 0
            HalfSpace(sparsevec([x, y], [tan30, 1.0], n), 0.0)]   # -x tan(30°) - y >= 0
    property_rendezvous = is_contained_in(HPolytope([octagon; cone]))
    # safety property in "aborting"
    target = HPolytope([HalfSpace(sparsevec([x], [-1.0], n), 0.2),  # x >= -0.2
                        HalfSpace(sparsevec([x], [1.0], n), 0.2),   # x <= 0.2
                        HalfSpace(sparsevec([y], [-1.0], n), 0.2),  # y >= -0.2
                        HalfSpace(sparsevec([y], [1.0], n), 0.2)])  # y <= 0.2
    ...
    # default options
    options = Options(:T => 300.0, :property => property)
