...
function plot_dashed_lines!(v, time_horizon=20.0)
...
res = ReachSolution([ReachSet(Interval(time_start(rs), time_end(rs)) × set(rs), time_start(rs),
...
plot(res;
     ...
     xtick=[0.0, 10.0, 20.0], ytick=[-7e-4, -5e-4, 0.0, 5e-4, 7e-4],
     xlims=(0.0, 20.0), ylims=(-8e-4, 8e-4),
...
plot(res;
     ...
     xlab=L"t\raisebox{-0.5mm}{\textcolor{white}{.}}",
     ylab=L"y_3\raisebox{2mm}{\textcolor{white}{.}}",
     xtick=[0.0, 10.0, 20.0], ytick=[-17e-5, 0.0, 17e-5],
     xlims=(0.0, 20.0), ylims=(-2e-4, 2e-4),
     title=L"\textcolor{white}{.}\cdot 10^{-4}", title_location=:left,
     titlefont=font(35, "Times"),
